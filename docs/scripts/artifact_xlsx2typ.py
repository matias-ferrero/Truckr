...
def _gen_usm(sheet_name: str, output_name: str):
    ...
    # Map column -> color from task rows (scan all rows 3+ to handle columns whose
    # first task appears in a later release, e.g. Seguros or Reseñas sub-columns).
    # Only consider rows that have a non-empty cell value in this column (actual tasks).
    ...
    # Detect release section boundaries from column 0.
    # Any non-empty value in col 0 (rows 2+) is a release marker — includes "MVP",
    # "Release 2", "Release 3", etc.
    release_rows = []   # pandas row indices of marker rows
    release_labels = [] # exact label text from the XLSX
    for r in range(2, df.shape[0]):
        val = _clean(df.iloc[r, 0])
        if val:
            release_rows.append(r)
            release_labels.append(val)

    # Group tasks by release: tasks between the previous marker (exclusive) and
    # the current marker (exclusive).
    tasks_by_release = {}  # release_index -> {col -> [tasks]}
    for release_idx, release_row in enumerate(release_rows):
        tasks_by_release[release_idx] = {c: [] for c in range(1, df.shape[1])}
    ...
    # Generate rows for each release using the exact labels from the XLSX
    for release_idx in sorted(tasks_by_release.keys()):
        release_label = release_labels[release_idx]
        ...
        # Release marker row (spans all columns, matches XLSX label exactly)
        lines.append("")
        lines.append(
