...
| **Destino abierto** | — (concept / `:open_destination` factory trait) | `TransportWindow` con `destination_lat` / `destination_lng` ambos `NULL` (y `dropoff_radius_km` también `NULL` por la regla de NULL-juntos); el matcher omite el filtro de entrega y el Carrier acepta cargas a cualquier destino, siempre que la dirección de retiro caiga dentro del `pickup_radius_km` del origen de la ventana. La marca dejó de ser la ausencia de `destination_province` cuando ADR-014 eliminó esa columna; hoy es estrictamente `destination_lat / destination_lng IS NULL`. Introduced in FIX-BE-00001, redefined under ADR-014. |
| **Ventana de transporte** | `TransportWindow` / `transport_windows` | Block of availability published by a Transportista (origin, destination, time range, vehicle). Location fields (per ADR-014): `origin_address` (texto completo `formatted_address` de Google, para mostrar), `origin_locality` y `origin_admin_area` (parseados de `address_components` en el FE vía `frontend/src/lib/places.ts`, sólo para etiquetas de UI — nunca se consultan), `origin_lat` / `origin_lng` (`DECIMAL(9,6)`, autoritativos para matching), y `pickup_radius_km`; el mismo set para destino (`destination_address`, `destination_locality`, `destination_admin_area`, `destination_lat`, `destination_lng`, `dropoff_radius_km`), todo nullable en bloque cuando es *Destino abierto*. Los campos heredados `origin_province` / `destination_province` / `_locality` / `_normalized` fueron eliminados por ADR-014. |
...
| **Radio de entrega** | column: `dropoff_radius_km` on `transport_windows` | Distancia en km, anclada en el `destination_lat` / `destination_lng` publicado de la `TransportWindow`, simétrica a *Radio de recogida*. Expresa cuánto está dispuesto el Transportista a desviarse al entregar la carga. Rango 1–200 km. **`NULL` si y sólo si el destino es abierto** (`destination_lat` / `destination_lng` también `NULL`); las tres columnas se quedan `NULL` en bloque o todas con valor — nunca un mix. Usado por el matcher de US5 después del filtro de pickup: una Cargo es compatible sólo si su `delivery_lat` / `delivery_lng` cae dentro del radio respecto del destino de la ventana (Haversine en código de aplicación, mismo path que el pickup). Definido al publicar la ventana (US9 / US48) y editable vía US33 «Editar Ventana de Transporte». Cambios en el radio (subir o bajar) NO invalidan `CargoOffer`s ya en `pending` — mismo principio que *Radio de recogida*. Introducido por ADR-014. |
| **Carga** | `Cargo` / `cargos` | Goods published by an Expedidor for transport. A published `Cargo` is the unit Expedidores can search against and bid on (via a `CargoOffer`). Lifecycle: `open / accepted / cancelled`. Pickup and delivery addresses carry geocoded pins (`pickup_lat` / `pickup_lng`, `delivery_lat` / `delivery_lng`, `DECIMAL(9,6)`) sourced from the address picker on the publish form (US49), junto con el `pickup_address` / `delivery_address` (texto formateado) y los pares parseados `pickup_locality` / `pickup_admin_area` / `delivery_locality` / `delivery_admin_area` (extraídos de Google `address_components` en el FE, sólo para etiquetas de UI — nunca se consultan). Los campos heredados `pickup_zone` / `delivery_zone` / `_normalized` fueron eliminados por ADR-014. |
