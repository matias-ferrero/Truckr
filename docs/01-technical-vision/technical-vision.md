...
### ADR-012 — `Payment` model: per-attempt rows, terminal `escrowed`, fake gateway in production
...
- **`Payment` belongs to `Shipment`**, not `CargoOffer`. Documented `Shipment 1:N Payment`. Per-attempt rows: each "Pagar" click creates a new `Payment` row in `pending`. Failed attempts are kept (soft-deleted via the `discard` gem, per ADR-009) — supports forensics on "why did this Shipment have three failed charges?".
- **FSM (MVP)**: `pending → escrowed | rejected`. `escrowed` is terminal — there is no settlement job, no `released`, no `refunded`, no `disputed`. Re-introducing them requires a real-gateway integration and is out of scope.
...
- **Shipper-initiated abandonment**: `POST /api/payments/:id/abandon` transitions a stuck `pending` row to `rejected`. There is no timeout job — Shipper recovery is manual but in-app. Reason: timeout was explicitly out of scope (verbal decision 2026-05-22) to keep US8 small.
- **No "paid" state on `Shipment`**. The Shipment FSM (`draft → offered → accepted → in_transit → delivered → settled`) is untouched. "A recoger" (AC2 wording) is a UI-derived label gated on `shipment.payments.escrowed.exists?`, not a canonical state. Rationale: keeps the Shipment FSM aligned with physical events.
...
- **API surface**: `POST /api/shipments/:id/payments` (create), `GET /api/payments/:id/return?outcome=approved|rejected|pending` (gateway return URL, idempotent — wraps state transition + Shipment-side effects in a `with_lock`), `POST /api/payments/:id/abandon` (manual recovery). No webhook endpoint.
...
- Doc surface updated in lockstep: `domain-model.md` § 1 (1:1 → 1:N), § 3 (drop "open Payment row" side effect on accept), § 4.1 (add "payment as predicate" callout), § 5.1 (FSM reduced); `api-overview.md` (new routes, webhook removed); `external-systems.md` (fake gateway is the production gateway); `scheduled-jobs.md` (`PaymentSettlementJob` removed); `glossary.md` (`Pasarela falsa` added).
- Post-MVP delta is concrete and small: add `released` + `refunded` + `disputed` to the FSM, add `PaymentSettlementJob` and the dispute flow, add a real-gateway adapter (`Payments::MercadoPagoGateway`), keep the fake one for QA / e2e tests.
