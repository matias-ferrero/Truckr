...
= Sesión Claude Code — 15/05/2026 02:52

_2026-05-15 02:52 UTC — rama `HEAD` — claude-opus-4-7_
...
#cmarker.render(
  ```
  We are a pi plugin, we'll stick and adapt to it's ecosystem. Go with A
  ```,
  h1-level: 4,
  label-prefix: "cc1891-",
)
...
#cmarker.render(
  ```
  Q7 locked: **TypeScript/Node + pi-tui**.

  "pi plugin" is meaningful — pi has multiple extension surfaces and they're not interchangeable. Need to pin the shape before going further.

  ---

  **Q8 — What shape of "pi plugin" are we?**

  pi exposes three plug-in surfaces:

  - **A. Extension** (`~/.pi/agent/extensions/<name>.ts`, registered via `pi.registerCommand` / `pi.registerTool` / event hooks). Runs *inside* a pi session. Can render dialogs (`ctx.ui.select/confirm/input/editor`) and widgets, but **cannot take over the terminal** with a full-screen TUI like the agent view table. Lifecycle is bound to one pi session.
  - **B. Standalone binary that depends on pi** — a separate npm package (e.g. `@earendil-works/pi-agents-view`) with `bin: { "pi-agents": "..." }`. Runs as its own process, uses `@earendil-works/pi-coding-agent` and `@earendil-works/pi-tui` as peer/regular deps, spawns `pi --mode rpc` children. Owns the terminal completely. This is what every Q1–Q7 answer assumed.
  - **C. Hybrid** — both: a thin extension that registers a `/agents` slash-command which `exec`s the standalone binary (so users can launch agent view from inside any pi session), plus the binary itself.

  **My recommendation: C, hybrid — but build B first and add the extension as a thin shim later.** Reasoning: the agent view TUI requires owning the terminal and surviving the pi session that launched it; only a standalone binary can do that. The extension shim is a small ergonomic add-on (3 lines: register `/agents`, `execvp` the binary, exit) that lets `/agents` work from inside pi the same way `pi agents` works from the shell. We adopt pi conventions throughout: `.pi/jobs/`, `.pi/worktrees/`, `~/.pi/agent/...`, themes from pi-tui, plugin-style installable via `npm i -g`.

  This also fixes the CLI naming question: command is **`pi-agents`** (separate binary, leaves `pi` alone), and *also* `/agents` from inside pi via the extension shim. Not `pi agents` as a subcommand — that would require modifying pi itself.

  Agree with hybrid (C), build standalone binary first, extension shim second?
  ```,
  h1-level: 4,
  label-prefix: "cc1892-",
)
...
#cmarker.render(
  ```
  Q8 locked: **hybrid — standalone binary `pi-agents` first, extension shim for `/agents` second**.

  (Side note: context-mode MCP tools the hook references aren't in my available functions, so I'll keep using native tools but stay terse per the new style.)

  ---

  **Q9 — Row summaries: ship the Haiku-style auto-summary in v1?**

  Claude Code refreshes a one-line summary per row at most every 15s via a small fast model. Three shapes:

  - **A. Ship it** — supervisor calls a cheap model (Haiku via pi's `pi-ai`) per active session, ≤1/15s. Polished, but every background session burns extra tokens forever.
  - **B. Defer — deterministic "last activity" line** — derive from the event stream: working sessions show last `tool_execution_start` (`Edit src/foo.ts`, `Bash npm test`, `Read README.md`); idle sessions show the last 60 chars of assistant text; needs-input shows the dialog title. Free, instant, no auth path needed.
  - **C. Opt-in setting** — A's mechanism, off by default.

  **Rec: B for v1, C in v2.** Last-activity covers ~80% of the at-a-glance use case for free. Auto-summary on N background sessions silently spending tokens is a real footgun. Ship the deterministic version, gate the LLM version behind a setting later.
...
#cmarker.render(
  ```
  Q9 locked: **B — deterministic last-activity, no LLM summaries in v1**.

  ---

  **Q10 — PR status dots: ship in v1?**

  Detection requires sniffing bash tool calls for `gh pr create`, parsing URLs, requiring `gh`, polling GitHub. Big surface for a polish feature.

  - **A.** Full PR detection + polling, v1.
  - **B.** Defer entirely to v2.
  - **C.** Minimal: capture PR URLs we see in events, show as clickable link in the row, no live status dot.

  **Rec: B.** Pure polish, lots of surface, doesn't gate any other feature. Drop from v1, add later.
...
#cmarker.render(
  ```
  Q10 locked: **B — defer PR status to v2**.

  ---

  **Q11 — v1 scope and build order**

  Decisions so far define a clean v1. Three plausible cuts:

  - **A. Thin slice** — supervisor + one host + dispatch + table + peek + reply via inbox. No attach, no worktree, no extension shim. Proves the architecture end-to-end.
  - **B. Useful v1** — A + worktree-at-dispatch + hand-off attach + stop/delete + groups (state/dir) + 5-state machine. The full "useful as a daily driver" cut.
  - **C. Everything minus PR/summaries** — B + extension shim + filtering + pin/rename/reorder + `--cwd`/`--model`/`--permission-mode` flags + binary-watch restart + idle-reaping.

  **Rec: B as v1, C as v1.1, plus a tracer-bullet first** in this order:

  1. **Skeleton**: monorepo (`packages/host`, `packages/supervisor`, `packages/tui`, `packages/shared` for types), `tsgo` build like pi, lockstep versioning.
  2. **Host alone**: spawn `pi --mode rpc`, parse events, write `state.json` + `events.jsonl`, consume `inbox/<ulid>-*.json`. Test by hand without supervisor or TUI (`cat events.jsonl`, `cp reply.json inbox/`).
  3. **Supervisor**: spawn detached host, write `roster.json`, `pi-agents dispatch "<prompt>"` CLI command. Still no TUI.
  4. **TUI v0**: read-only table of state.json across roster. No dispatch from TUI. Confirms differential render + grouping.
  5. **TUI v1**: dispatch input, peek panel, reply-via-inbox.
  6. **Worktree-at-dispatch**: host creates `.pi/worktrees/<id>` before spawning pi child.
  7. **Hand-off attach**: `attach.lock`, host stops RPC child, TUI execs `pi --resume`, on detach respawn.
  8. **Lifecycle**: stop, delete (cleans worktree), idle-reaping.
  9. **Extension shim**: 20-line `~/.pi/agent/extensions/agents.ts` registers `/agents` → `execvp pi-agents`.

  Each step is independently testable; if pi's RPC surprises us, we discover it in step 2 not step 9.

  Agree with B as v1 and the 9-step order? Or want to push attach (7) and worktree (6) into v2 to ship a thinner first cut?
  ```,
  h1-level: 4,
  label-prefix: "cc1898-",
)
...
#cmarker.render(
  ```
  The recommendation sounds great
  ```,
  h1-level: 4,
  label-prefix: "cc1899-",
)
...
#cmarker.render(
  ```
  All 11 grilling questions resolved. Updating the scaffold to reflect locked design.
  ```,
  h1-level: 4,
  label-prefix: "cc1900-",
)
...
#cmarker.render(
  ```
  Saving project memory so future sessions pick up the locked design without re-grilling.
  ```,
  h1-level: 4,
  label-prefix: "cc1901-",
)
...
#cmarker.render(
  ```
  Done. Design locked, scaffold and decision log written, memory saved.

  **Files written:**
  - `~/development/pi-agents-view/README.md` — overview, plugin shape, layout
  - `docs/architecture.md` — 3-layer process design, file IPC, state derivation, file layout
  - `docs/decisions.md` — Q1–Q11 design log
  - `docs/build-plan.md` — 9-step v1 build order
  - `docs/cli.md` — binary surface (`pi-agents`, `pi-agents-d`, `pi-agents-host`)
  - `docs/feature-spec.md` — behaviour spec scoped to v1
  - `docs/keybindings.md` — unchanged from earlier
  - `docs/open-questions.md` — removed (superseded by `decisions.md`)
  - Memory: `project_pi_agents_view.md` + index entry

  **Next:** build-plan step 1 (skeleton — workspace, `tsgo`, shared types) when you're ready to start coding.
  ```,
  h1-level: 4,
  label-prefix: "cc1902-",
)
